...
<v-content>
  <v-card style="padding: 10px">
    <v-card-title style="font-size: 30px;">Review {{userData.first_name + " " + userData.last_name}}
    </v-card-title>
    <v-row>
      <v-chip style="margin-left: 20px;" outlined>
        <v-icon style="margin-left: 5px; margin-right: 5px; padding: 0;" v-for="num in [1,2,3,4,5]"
                :key="num" color="starColor" v-on:click="setStar(num)">{{rating >= num ? 'mdi-star' :
          'mdi-star-outline'}}
        </v-icon>
      </v-chip>
    ...
    <v-row>
      <v-textarea required :rules="value => !!value || 'Required.'" outlined style="margin: 0; padding: 10px;"
                  type="text" label="Review" placeholder="Share some words about your experience"
                  v-model="review"></v-textarea>
    </v-row>
    <v-row>
