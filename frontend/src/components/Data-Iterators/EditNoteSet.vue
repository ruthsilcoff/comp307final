...
<v-container>
  ...
  >
    ...
    <div style="margin: 0; padding: 5px;" v-if="search && (search !== '') ">
      ...
      <div style="margin: 0; padding: 0;">Press <kbd
          v-on:click="addNewSubject({name: search})">enter</kbd> to create a new one
      </div>
    </div>
    <div style="margin: 0; padding: 5px;"
         v-if="(search === '' || !search) && (subjectsInput.length === 0) && (subjectsNotIncluded.length === 0)">
  ...
  <div class="noHoverDrop" style="border: 2px dashed black" id="dragDropPhotoBox" v-cloak @drop.prevent="addFile"
       @dragover.prevent="activateDragOver" @dragleave="deactivateDragOver">
    <h3>{{text}}</h3>
        ...
        if (!this.relatedSubjects.includes(this.subjectsInput[i])) {
          if (this.allSubjectNames.includes(this.subjectsInput[i])) {
            ...
            try {
              ...
              this.createSnackbar({
                message: "New subject created and added to list",
                color: 'success'
              })
            } catch (error) {
              this.createSnackbar({message: "Problem adding subject", color: 'error'})
