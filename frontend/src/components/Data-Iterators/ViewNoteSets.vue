<template>
  <v-content style="padding: 0;">
    <v-toolbar style="margin-bottom: 30px;" fixed absolute width="100%" floating  color="primarydarken1" >
        ...
        <v-spacer></v-spacer>
        <v-btn style="padding: 0;" icon v-if="!isViewing && !addingNoteSet" v-on:click="openNewNoteSet" color="success" large>
                    ...
                    <v-list-item  class="listHighlight" link>
                        <div style="display: flex; position: absolute !important; top:0; right: 0;">
                          <router-link :to="'/noteSet/' + item.id">
                            <v-btn title="view note" style="margin:0; padding: 0;" v-if="!isViewing" icon color="info"><v-icon>mdi-eye</v-icon></v-btn>
                          </router-link>
                          <v-btn title="delete note" style="margin:0; padding: 0;" v-if="!isViewing" icon color="error" v-on:click="removeNote(item)"><strong>X</strong></v-btn>
                        </div>
